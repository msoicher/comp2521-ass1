...
{
	assert (g != NULL);
	//  array whther ith vertex is reached at least once
	int visited[g->nV];

	// setting all to 0 (not visited)
	for (int i = 0; i < g->nV; i++)
		visited[i] = -1;

	int found = -1;

if (g->edges[src][dest] <= max && g->edges[src][dest] >= 0) {
   path[0] = src;
   path[1] = dest;
   return 2;
}
	...
	while (found == -1 && !QueueIsEmpty(q)) {
		...
		if (v == dest) {
			...
			for (int j = 0; j < g->nV; j++) {
				if (visited[j] == -1 && g->edges[v][j] <= max) {
					visited[j] = v;
					QueueJoin(q, j);
					// If it is destination then break;
					if (j == dest) {
						break;
					}
				}
